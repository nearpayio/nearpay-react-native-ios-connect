...
var _responseParser = require("./Utils/responseParser");
...
class NearpayConnect {
  ...
  onPurchase(callback) {
    this.emitter.addListener('onStartPurchase', _NearpayEventEmitter.EventType.terminal, result => {
      try {
        const transactionModel = (0, _responseParser.parseAndTransformJOBResponse)(result);
  ...
  onReverse(callback) {
    this.emitter.addListener('onStartReverse', _NearpayEventEmitter.EventType.terminal, result => {
      try {
        const transactionModel = (0, _responseParser.parseAndTransformResponse)(result);
        callback(transactionModel);
      } catch (error) {
  ...
  onReconciliation(callback) {
    this.emitter.addListener('onStartReconciliation', _NearpayEventEmitter.EventType.terminal, result => {
      try {
        const reconciliationModel = (0, _responseParser.parseAndTransformJOBResponse)(result);
  ...
  verify(otp, timeout) {
    return this.auth.verify(otp, timeout).then(response => {
      return (0, _responseParser.parseAndTransformResponse)(response);
    });
  }
